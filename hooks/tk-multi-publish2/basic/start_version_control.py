...
class SubstanceDesignerStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the substancedesigner file path if one
    does not exist.
    """
    ...
    def item_filters(self):
        ...
        """
        return ["substancedesigner.session"]
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "SubstanceDesigner '%s' plugin rejected the current session..."
                    % (self.name,)
                )
                self.logger.info("  There is already a version number in the file...")
                self.logger.info("  SubstanceDesigner file path: %s" % (path,))
            ...
            self.logger.warn(
                "The SubstanceDesigner session has not been saved.",
                extra=_get_save_as_action(),
            )

        self.logger.info(
            "SubstanceDesigner '%s' plugin accepted the current session."
            % (self.name,),
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            # validation fails
            error_msg = "The SubstanceDesigner session has not been saved."
    ...
    def publish(self, settings, item):
        ...
        self.logger.info(
            "A version number has been added to the SubstanceDesigner file..."
        )
        self.logger.info("  SubstanceDesigner file path: %s" % (version_path,))
